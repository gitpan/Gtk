...
GtkType FindArgumentType(GtkObject * object, SV * name, GtkArg * result) {
	...
	if (t == GTK_TYPE_INVALID)
		croak("Unknown argument %s of %s", SvPV(name,PL_na), 0 ? "(none)" : gtk_type_name(object->klass->type));
...
void GtkSetRetArg(GtkArg * a, SV * v, SV * Class, GtkObject * Object)
{
	...
	if (!result) {
		struct PerlGtkTypeHelper * h = PerlGtkTypeHelpers;
		while (!result && h && h->GtkSetRetArg_f) {
...
SV * GtkGetRetArg(GtkArg * a)
{
	...
	if (!result) {
		struct PerlGtkTypeHelper * h = PerlGtkTypeHelpers;
		while (!result && h && h->GtkGetRetArg_f) {
